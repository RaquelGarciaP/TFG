...
def keplerian_orbit(t, rv0, period, K, ecc, omega, t_peri):
    ...
    #   Input parameters:
    #      t: array of time
    #      rv0: systemic radial velocity (velocitat del sistema respecte el Sol; en el nostre cas rv0 = 0)
    ...
    omega = omega * np.pi / 180.0  # passant omega a radiants
    sinf, cosf = true_anomaly(t, period, ecc, t_peri)
    cosftrueomega = cosf * np.cos(omega) - sinf * np.sin(omega)  # cos(f+omega) = cosf*cos(omega) - sinf*sin(omega)
...
def true_anomaly(t, period, ecc, t_peri):
    ...
    for i in range(len(t)):
        fmean = 2.0 * np.pi * (t[i] - t_peri) / period  # anomalia mitjana
        # Solve by Newton's method x(n+1)=x(n)-f(x(n))/f'(x(n))
        fecc = fmean
        diff = 1.0
        while diff > 1.0E-6:
            fecc_0 = fecc
            fecc = fecc_0-(fecc_0-ecc*np.sin(fecc_0)-fmean)/(1.0-ecc*np.cos(fecc_0))  # anomlaia excentrica
            diff = np.abs(fecc-fecc_0)
        sinf.append(np.sqrt(1.0-ecc*ecc)*np.sin(fecc)/(1.0-ecc*np.cos(fecc)))  # sin anomalia veritable
        cosf.append((np.cos(fecc)-ecc)/(1.0-ecc*np.cos(fecc)))  # cos anomalia veritable
